...
def diet_list(request):
    ...
    diets_queryset = Diet.objects.filter(patient__nutritionist=request.user).select_related('patient__user').order_by('-created_at')
...
def diet_create(request, patient_pk=None):
    ...
    if request.method == "POST":
        try:
            ...
            messages.error(request, f'Erro ao salvar plano alimentar: {str(e)}')
...
def buscar_alimentos(request):
    ...
    # Buscar no banco de dados TACO
    alimentos = AlimentoTACO.objects.filter(
...
def gerar_pdf(request, patient_id):
    ...
    try:
        patient = get_object_or_404(PatientProfile, pk=patient_id, nutritionist=request.user)
